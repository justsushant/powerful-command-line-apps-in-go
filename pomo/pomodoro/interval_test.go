...
func TestGetInterval(t *testing.T) {
	...
	for i := 1; i <= 16; i++ {
		...
		// For each test, start the interval and test that the category and expected duration matches the expected values, 
		// failing the test if they don’t match. 
		// Because we’re not interested in what happens during the interval execution, 
		// define an empty function (noop) to use as a callback and use an empty context as the context
